...
def convexo():
    ...
    y = 0.0
    while t <= 1:
        for x in arange(0.0,25.0, 0.1):
            for y in arange(0.0,25.0,0.1):
                hessiana = hes(x, y)
                if isnan(hessiana[0][0]): #caso em que x = -y ou vice versa e, portanto, o cálculo da hessiana terá divisão negativa, indicando um ponto de descontinuidade da função.
